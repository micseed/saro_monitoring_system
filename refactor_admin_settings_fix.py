...
# 4. Replace the entire section-password HTML
...
# 5. Replace Modal Block using string split
marker = "<!-- -- Password Request Modal -- -->"

if marker in content:
    pre_content, post_content = content.split(marker, 1)
    # the modal and original script ends at </script>
    if "</script>" in post_content:
        _, after_script = post_content.rsplit("</script>", 1)
...
"""
        content = pre_content + new_script_block + after_script

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)
print("Updated admin settings successfully.")
