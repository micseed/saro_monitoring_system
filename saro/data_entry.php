...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'delete') {
        ...
        $saroRecord = $saroObj->getSaroById($id);
        if ($saroRecord && in_array($saroRecord['status'], ['obligated', 'lapsed'], true)) {
            echo json_encode(['success' => false, 'error' => 'Obligated and lapsed SAROs cannot be deleted.']);
            exit;
        }
    ...
    if ($action === 'cancel') {
        ...
        $saroRecord = $saroObj->getSaroById($id);
        if ($saroRecord && in_array($saroRecord['status'], ['obligated', 'lapsed'], true)) {
            echo json_encode(['success' => false, 'error' => 'Obligated and lapsed SAROs cannot be cancelled.']);
            exit;
        }
